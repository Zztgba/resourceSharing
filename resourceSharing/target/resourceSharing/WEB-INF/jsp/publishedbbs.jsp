...
<body>
<div class="page_nav">
    <p>当前位置：<span>帖子管理</span>/<span>已审核帖子</span></p>
</div>
    ...
    <table>
        ...
        <c:forEach items="${list}" var="bbsVo" varStatus="vs">
            <tr>
                ...
                <td>${bbsVo.name}</td>
                <td><fmt:formatDate value="${bbsVo.notetime}" pattern="yyyy-MM-dd hh:mm:ss"/></td>
                <td><a href="${pageContext.request.contextPath}/BbsManager/updateBbsState/2.action?id=${bbsVo.id}&&index=2">撤销</a></td>
            </tr>
        </c:forEach>
        </thead>
    </table>
